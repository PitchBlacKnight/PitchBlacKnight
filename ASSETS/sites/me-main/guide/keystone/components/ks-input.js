...
sheet.replaceSync(`
  :host { display: block; }
  .field { display: flex; flex-direction: column; gap: var(--primitive-space-2, 8px); }
  ...
  .req { color: var(--semantic-danger, #c0392b); margin-inline-start: 2px; }
  ...
  input {
    ...
    color: var(--component-input-fg, #1a1917);
    font-family: var(--primitive-font-family-sans, system-ui);
    font-size: var(--primitive-font-size-sm, 14px);
    box-shadow: inset 0 0 0 var(--primitive-border-width-thin, 1px) var(--component-input-border, #d9d5cc);
    transition: box-shadow var(--component-input-motion, 120ms) cubic-bezier(.2,0,.1,1);
  }
  input::placeholder { color: var(--component-input-fg-placeholder, #5c564e); }
  input:hover { box-shadow: inset 0 0 0 1px var(--component-input-border-hover, #9a938a); }
  input:focus-visible {
    outline: var(--primitive-border-width-focus, 2px) solid var(--component-input-focus-ring, #c8552b);
    outline-offset: 0;
  }
  :host([disabled]) input {
    background: var(--component-input-bg-disabled, #edebe6);
    color: var(--semantic-text-disabled, #9a938a);
    cursor: not-allowed;
  }

  /* Invalid is signalled by border + icon + text — never by colour alone */
  :host([data-invalid]) input {
    box-shadow: inset 0 0 0 var(--primitive-border-width-focus, 2px) var(--component-input-border-invalid, #c0392b);
  }
  ...
  .msg.error {
    color: var(--semantic-danger, #c0392b);
...
class KsInput extends HTMLElement {
  ...
  constructor() {
    ...
    this._input.addEventListener('input', () => {
      ...
      this.dispatchEvent(new CustomEvent('ks-input', { detail: { value: this._input.value }, bubbles: true, composed: true }));
    });
  }
  ...
  _sync() {
    ...
    if (!label) console.warn('<ks-input> is missing a label. A placeholder is not a label.');
