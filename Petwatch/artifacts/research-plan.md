...
The PRD makes a strong claim: that PetWatch as specified is a publishing tool, and
that completion + activity is what turns it into a trust instrument.
...
### A1 - Owners want confirmation, and watchers don't experience it as surveillance
...
The tension is real. The watcher is doing a *favor*. Anything that reads as
...
visibility. The watcher's reaction is the finding.
...
- Concept test two variants after the retrospective, never before:
  - **V1 active:** watcher ticks tasks off; owner sees an itemized feed
  - **V2 passive:** watcher sees the checklist; owner sees only "last activity 7:12am"
- The question that matters, asked of the watcher: *"If your friend could see this,
  would you still want to use it?"*

**Decision rule, written before the research so I cannot rationalize afterwards:**
- 4 or more of 6 watchers react neutrally or positively to V1 → build V1
...
**Method - instrumentation, not interviews.** This one answers itself for free.
...
**Method - artifact study, 8 owners, asynchronous, ~20 minutes of my time each.**
The cheapest and highest-yield study in this plan:
...
### A4 - Today-first is the right default for watchers
...
| Week 0 - in parallel | **A3 artifact study** (async, low effort) | The schedule data model |
...
| Cycle 1 | A2 + A5 instrumentation ships with the features | - |
...
- **Quarterly re-interview** of 3 active owner/watcher pairs - the relationship
...
- **A survey.** "Would you like to see when tasks are completed?" produces a yes from
  everyone and tells us nothing. The question is how the *watcher* feels, and nobody
