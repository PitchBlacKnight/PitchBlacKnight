...
**Not in the order the stories are written.** The brief groups them by feature -
logins, pets, schedules, invitations - which is a fine way to *write* requirements
and a bad way to *build*. Go feature by feature and you'll have a finished login
system and a finished pet manager before any human has ever actually used the
product - meaning you learn whether the idea works at the very end, when changing
course is most expensive.

**Instead: tackle the scariest unknowns first.** The first release is deliberately
bare-bones but complete - one owner can hand one pet to one sitter and get through
a real trip, start to finish. Thin and unglamorous, but it's the fastest way to
watch real people actually use the thing, which is the only way to find out if the
big idea is right.

Three ordering principles:

1. **Build the riskiest idea first, not best.** Task check-offs land in release one
   in their roughest form. If people hate feeling watched, I want to know in week 3,
   not after we've spent months polishing it.
2. **Nothing is scheduled that cannot be demoed.** Every cycle ends with something
   a person can use in front of us on Friday.
3. **Measurement is built in from day one, never bolted on.** The counters that
   tell us how a feature is doing ship inside the same ticket as the feature.
   "We'll add tracking later" is a promise no team has ever kept.

---

## Release 1 - "The Handoff" · Cycles 1–3

**Goal:** One owner can hand one pet to one sitter, and both can complete a real
...
| PW-7 | Seeded staging fixtures: pet with a full week, pending invite, revoked sitter | 2 | `devops` `qa` |
...
> sitter and the expired invite to exist on staging from week one.
...
> **PW-14 is the ticket everything else stands on.** Everything in Today and
...
| PW-22 | Sitter list per pet; revoke access; history retained | 3 | `frontend` `backend` |
...
| ID | Ticket | Est | Labels |
|---|---|---|---|
| PW-25 | Sitter reminders - task due, quiet hours respected | 5 | `backend` `frontend` |
...
| PW-33 | Sitter-side multi-owner view - watching for 2+ households | 3 | `frontend` |
