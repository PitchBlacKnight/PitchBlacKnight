...
- **Every UI ticket carries acceptance criteria I co-wrote with QA**, and those
  ...
  "empty, loading, error, offline, and the two-sitters-collide case" is.
...
- **Analytics events are acceptance criteria on the feature ticket.** A separate
  separate "add tracking later" ticket is a ticket that never gets built.
...
- **The prototype is the spec for interaction.** Static redlines cannot describe
  a check-off that happens instantly with five minutes to undo it. A working prototype can.
...
of tasks, a pending invite, an expired invite, and a revoked sitter on staging
...
the honesty checks - especially how often someone ticks a whole day's tasks in one late-night burst. I want the number that tells
