...
- **The central thesis.** The completion gap in the PRD, The gap in the current scope - the observation that the
  user stories describe a product where nothing can ever be marked done, and that this is a
  ...
  a model produces login flows and standard app plumbing, because things that are *missing* don't
...
- **The surveillance counter-argument.** Having found "add completion," the
  reflexive next step is to design it. Stopping to ask *whether the sitter would
...
2. **The first database draft pre-filled a table with every future feeding** -
